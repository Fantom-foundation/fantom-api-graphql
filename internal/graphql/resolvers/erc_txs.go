...
func (rs *rootResolver) Erc20Transactions(args struct {
	...
	tl, err := repository.R().TokenTransactions(types.AccountTypeERC20Token, args.Token, nil, args.Account, &txType, (*string)(args.Cursor), args.Count)
	if err != nil {
		return nil, err
	}
...
func (rs *rootResolver) Erc721Transactions(args struct {
	...
	// get the transaction hash list from repository
	txType := ercTrxTypeFromName(args.TxType)
	tl, err := repository.R().TokenTransactions(types.AccountTypeERC721Contract, args.Token, (*big.Int)(args.TokenId), args.Account, &txType, (*string)(args.Cursor), args.Count)
	if err != nil {
		return nil, err
	}
...
func (rs *rootResolver) Erc1155Transactions(args struct {
	...
	// get the transaction hash list from repository
	txType := ercTrxTypeFromName(args.TxType)
	tl, err := repository.R().TokenTransactions(types.AccountTypeERC1155Contract, args.Token, (*big.Int)(args.TokenId), args.Account, &txType, (*string)(args.Cursor), args.Count)
	if err != nil {
		return nil, err
	}
